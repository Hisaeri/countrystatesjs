...
var ppList = {
    ...
    "PrachuapKhiri": "Khan=ประจวบคีรีขันธ์",
    "Ranong": "ระนอง",
    "Ratchaburi": "ราชบุรี",
    "Rayong": "ระยอง",
    "RoiEt": "ร้อยเอ็ด",
...
function addDistrict(obj) {
    for (var i = 0; i < states.length; i++) {
        if (states[i].name === obj.province) {
            states[i].districts.push({ "district": obj.district, "postal_code": obj.postal_code });
            return true;
        }
    }
    states.push({
        "name": obj.province,
        "districts": [{ "district": obj.district, "postal_code": obj.postal_code }]
    });
    return false;
}
...
function formatData() {
    for (var i = 0; i < data.length; i++) {
        var obj = data[i];
        if (!addDistrict(obj)) {
            console.log('new province has been added : ', obj.province)
        }
    }
}
...
try {
    ...
    fs.appendFile('formatted_th_provinces.json', JSON.stringify({ states }), function (err) {
        if (err) throw err;
        console.log('Saved!');
    });
} catch {
